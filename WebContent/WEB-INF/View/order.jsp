...
<html>
    ...
    <body>
        ...
        <c:if test="${role == Role.ADMIN}">
            <h2><a href="${pageContext.request.contextPath}/delayed-orders">Задержанные заказы</a></h2>
            <h2><a href="${pageContext.request.contextPath}/date-orders">Заказы к дате</a></h2>
            <h2><a href="${pageContext.request.contextPath}/client-orders">Заказы клиента</a></h2>
        </c:if>
        <c:if test="${role == Role.USER}">
            <form action = "add-order" method = "POST" autocomplete="off">
                <label>Дата доставки:
                ...
                <label>Добавить заказ: <input type="submit" value="Добавить"></label>
            </form>
        </c:if>
        <table>
            <tr>
